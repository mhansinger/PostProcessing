...
mypath = 'postProcessing/sampleDict_Jsgs/'#input('Where is the extracted radial data? : ')
...
# read in the files
for file in files:
     if file.endswith("0.txt"):         # to exclude the TNF Data with different headers
...
def plotRadialCompare(field1='Dft',field2='Df'):
    plt.close('all')
    ...
    for i in range(len(radialAv)):
        ...
        # LES data
        plt.plot(thisData['x_Pos'], thisData[field1],'r', lw=1.6)
        plt.plot(thisData['x_Pos'], thisData[field2], 'b', lw=1.6)
        #try:
        #    if field == 'f_Bilger':
        #plt.plot(thisData['x_Pos'], thisData[field + 'RMS'], 'r', lw=1.6)
        #except:
        ...
        #plt.plot(thisExp['xmm mean'], thisExp[fieldE + ' RMS'], 'bo', lw=1)
        plt.xlim([0, 18])
        plt.title(field1+ ' and '+ field2 +' at Plane x/D = ' + location_dict[i])
        plt.ylabel(field1)
        plt.xlabel('Radius [mm]')
        plt.legend([field1,field2])
       # plt.legend(['LES mean','LES RMS','Exp. mean','Exp RMS'])
        plt.grid()
        fig_name = field1+'_'+location_dict[i]+'.png'
        #plt.savefig(fig_name)
