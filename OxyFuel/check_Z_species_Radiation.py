...
def plot_Radiation(sampleSize=50000):
    ...
    for i in range(len(scatterPlanes)):
        plt.figure(i + 1)
        thisData = LESdata[scatterPlanes[i]].sample(sampleSize)
        thisData_mole = compute_mol_fractions(thisData)
        T = thisData_mole['T']

        # absorption coefficients x molar fraction of species
        Q_CO2 = thisData_mole['x_CO2'] * ap_CO2(T)
        Q_H2O = thisData_mole['x_H2O'] * ap_H2O(T)
        Q_CH4 = thisData_mole['x_CH4'] * ap_CH4(T)
        Q_CO = thisData_mole['x_CO'] * ap_CO(T)

        thisData_mole['Q_radiation'] = 4*sigma*(T**4- T_b**4)*(Q_CO2 + Q_H2O + Q_CH4 + Q_CO)

        plt.scatter(thisData_mole['f_Bilger'],thisData_mole['Q_radiation'],s=0.2)

        plt.xlabel('Mixture fraction')
        plt.ylabel('Q_radiation [W]')
        plt.xlim(0, 0.3)
        #plt.plot([Z_st,Z_st],[0,2500],'--',lw=0.5)
        minSpec = min(thisData_mole['Q_radiation']) * 0.999
        maxSpec = max(thisData_mole['Q_radiation']) * 1.05
        plt.ylim(minSpec, maxSpec)
        #plt.legend(['Bilger','Species','Z_st'])
        plt.title('Scatter plot at: x/D=' + str(int(location_dict[i]) ))
