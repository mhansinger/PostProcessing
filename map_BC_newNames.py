...
data_points_org =1280 # 3300
data_points_new = 1533

idEnd = 40000       #ANPASSEN an precursor data 20001steps=0.0005s
...
def writeU(data_points=data_points_new,pfy=None,pfz=None,y=None,z=None):
    ...
    for id in range(idStart,idEnd):
        ...
        try:
            Ux, Uy, Uz = readPointsVector(filename=U_path,header=10,data_points=data_points_org)
        except IndexError:
...
def writeScalar(data_points=data_points_new,scalar = 'f', scalar_out='CH4',pfy=None,pfz=None,y=None,z=None):
    # do grid interpolation and write the data to new U files
    names = os.listdir('BC_precursor/FUEL/')
    names = [f for f in names if os.path.isdir('BC_precursor/FUEL/')]
    names.sort()
    ...
    for id in range(idStart,idEnd):
        ...
        try:
            # this is the value you want to interpolate onte the coarse mesh
        ...
        writePointsScalar(header_name='headerScalar.txt',out_path=out_path+'/'+scalar_out,Value=pfScalar)
        # print progress
...
x, y, z = readPointsVector('points_precursor', header=21, data_points=data_points_org)

# read in the patch field
pfx, pfy, pfz = readPointsVector('points_new', header=21, data_points=data_points_new)
...
writeScalar(data_points=data_points_new,scalar='f', scalar_out ='CH4', pfy=pfy,pfz=pfz,y=y,z=z)
writeScalar(data_points=data_points_new,scalar='f', scalar_out ='O2', pfy=pfy,pfz=pfz,y=y,z=z)
