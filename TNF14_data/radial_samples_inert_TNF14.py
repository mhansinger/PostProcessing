...
def radial_samples_inert(case_path='.'):
    ...
    scalarTail = '_TMean_TPrime2Mean_f_BilgerMean_f_BilgerPrime2Mean.xy'

    # '_f_BilgerMean_f_BilgerPrime2Mean_TMean_TPrime2Mean_CH4Mean_CH4Prime2Mean_O2Mean_O2Prime2Mean_CO2Mean_CO2Prime2Mean.xy'

    #scalarTail = '_TMean_TPrime2Mean_CH4Mean_CH4Prime2Mean_O2Mean_O2Prime2Mean.xy'
    ...
    for n in range(0, len(nLocation)):
        ...
        for time in times:
            for j in range(0, noFiles):
                try:
                    ...
                    # arrayO2 += dataScalar[:, 9]
                    # arrayO2RMS += np.sqrt(dataScalar[:, 10])
                    # arrayCO2 += dataScalar[:, 11]
                    # arrayCO2RMS += np.sqrt(dataScalar[:, 12])
                    # arrayJ_sgs += dataScalar[:, 11]
                    # arrayJ_sgsRMS += np.sqrt(dataScalar[:, 12])
                    # arrayJ_lam += dataScalar[:, 13]
                    # arrayJ_lamRMS += np.sqrt(dataScalar[:, 14])
        ...
        Output_np = np.array((arrayDist, arrayf, arrayT, arrayU, arrayV, arrayW, arrayfRMS, arrayTRMS,
                              arrayURMS, arrayVRMS, arrayWRMS,arrayJsgs_r,arrayJlam_r,arrayJsgsRMS_r,arrayJlamRMS_r,arrayVol,arrayCell))
        ...
        Output_df.columns = ['r_in_m', 'Z_mean', 'T_mean', 'U_axial_mean', 'U_radial_mean', 'U_teta_mean',
                             'Z_rms','T_rms', 'U_axial_rms', 'U_radial_rms', 'U_teta_rms',
                             'J_sgs_mean','J_lam_mean','J_sgsRMS_mean','J_lamRMS_mean','cellVolumes_mean','cellSize_mean']
